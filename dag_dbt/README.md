...
| `dag_dbt/`       | A Python directory that contains code for your Dagster repository                 |
| `dag_dbt_tests/` | A Python directory that contains tests for `dag_dbt`                      |
...
pyenv virtualenv $PYTHON_VERSION dag_dbt
pyenv activate dag_dbt
```

**Conda**
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name dag_dbt python=PYTHON_VERSION
conda activate dag_dbt
```
...
Tests can be found in `dag_dbt_tests` and are run with the following command:

```bash
pytest dag_dbt_tests
```

As you create Dagster ops and graphs, add tests in `dag_dbt_tests/` to check that your
